...
import QuestionGuide2 from './components/QuestionGuide2';
...
const Chat = ({
  ...
  const fetchData = (children: React.ReactNode) => {
    return <SideBar2>{children}</SideBar2>
  };
  useEffect(() => {
    // 使用DisplayComponent函数并传递子组件
    const componentToDisplay = fetchData(
      <QuestionGuide2 chatId={chatId} appId={appId} chatRecords={chatRecords} />
    );
    // 更新状态
    setDisplayComponent(componentToDisplay);
  }, [chatRecords]);
  return (
    <Flex h={'100%'}>
